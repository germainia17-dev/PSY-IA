  ...
  return (
    <View style={styles.wrap}>
      ...
      <View style={styles.grid}>
        {STARTERS.map((s, i) => (
          <Animated.View key={s.title} entering={reduced ? undefined : FadeInDown.delay(120 + i * 70).springify().damping(18)} style={styles.cell}>
            <Pressable
              onPress={() => onPick(s.prefill)}
              style={({ pressed }) => [
                styles.card,
                { backgroundColor: colors.surface, shadowColor: colors.shadow, opacity: pressed ? 0.7 : 1 },
              ]}>
...
const styles = StyleSheet.create({
  ...
  label: { fontSize: 11, fontFamily: 'Inter_600SemiBold', letterSpacing: 1.2 },
